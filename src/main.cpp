...
int main() {
    ...
    if (data == 1){
        ...
        if (iskyrimas == 1) {
            if (data == 1) { // list -> use strat_2
                skirstymas_pagal_paz_list(studentai_list, zem_lyg_list);
            } else { // vector -> use stable partition
                skirstymas_pagal_paz_vector(studentai_vec, zem_lyg_vec, aukst_lyg_vec);
            }
        }
        ...
        cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        if (data == 1) {
            handle_output(i_faila, iskyrimas, data, studentai_list, zem_lyg_list, aukst_lyg_list);
        } 
        else {
            handle_output(i_faila, iskyrimas, data, studentai_vec, zem_lyg_vec, aukst_lyg_vec);
        }
