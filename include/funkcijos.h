...
template <typename Container>
void skirstymas_pagal_paz_list(Container &studentai, Container &zem_lyg){
    auto start = std::chrono::high_resolution_clock::now();
...
template <typename Container>
void skirstymas_pagal_paz_vector(Container &studentai, Container &zem_lyg, Container &aukst_lyg){
    auto start = std::chrono::high_resolution_clock::now();

    // This function is intended for vectors only. Enforce at compile-time.
    static_assert(std::is_same<Container, std::vector<Studentas>>::value,
                  "skirstymas_pagal_paz_stable is vector-only; use strat_2 for lists");

    // VECTOR: use stable_partition (O(n)) — single pass, preserves order
    auto boundary = std::stable_partition(studentai.begin(), studentai.end(),
        [](const Studentas& s) { return s.galVid < 5.0; });

    // move low performers into zem_lyg, high performers into aukst_lyg
    std::move(studentai.begin(), boundary, std::back_inserter(zem_lyg));
    std::move(boundary, studentai.end(), std::back_inserter(aukst_lyg));
    studentai.clear();

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    std::cout << "Studentai surušiuoti (hybrid optimal) per: " << diff.count() << " s\n";
}
...
template <typename Container>
void handle_output(int i_faila, int iskyrimas, int data,
    ...
    if (i_faila == 1) {  // rasyti į failą
        ...
        } else if (data == 0) { //pirma strategija
            cout << "\nNeišlaikę studentai:\n";
            Outas_i_console(zem_container);
            cout << "\nIšlaikę studentai:\n";
            Outas_i_console(aukst_container);
        } else if (data == 1) { //antra strategija
